...
if [[ -n "$prev_trace_id" && -n "$prev_span_id" ]]; then
  ...
  attrs=$(jq -nc \
    ...
    '{"session.id":$sid,"trace.number":$num,"project.name":$proj,"openinference.span.kind":"LLM","output.value":"(Turn closed by fail-safe: Stop hook did not fire)"} + (if $in != "" then {"input.value":$in} else {} end) + (if $uid != "" then {"user.id":$uid} else {} end)')
...
# Jumi: the prompt carries injected task, feedback and CI prose. Store it only
# when the operator asked for prompt text; the turn span omits `input.value`
# otherwise.
prompt_value=""

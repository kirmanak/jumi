...
describe("reviewer image permissions", () => {
  ...
  test("does not pull base images from Docker Hub", () => {
    expect(dockerfile).toMatch(/FROM public\.ecr\.aws\/docker\/library\/debian:bookworm-slim(?:@sha256:[0-9a-f]+)? AS tools/);
    expect(dockerfile).toMatch(/FROM public\.ecr\.aws\/docker\/library\/debian:bookworm-slim(?:@sha256:[0-9a-f]+)? AS runtime/);

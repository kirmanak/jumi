...
- **Fail closed.** No refresh token, a refused grant, a write that will not land, or a verified refresh that still does not cover the job: the job fails as a provider auth death rather than starting a review that will 401 with nothing to recover with. A spawn on another provider only logs it.
...
Rotation does not revoke an access token already issued, so no lock is held for the length of a review: a later job may refresh while an earlier child is still working. The one accepted loss is a kill after xAI has answered and before the new file is durable; there is no second refresher to heal that.
